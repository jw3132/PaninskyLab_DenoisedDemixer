...
def simulate_missing_params(params):
    ...
    if simulate_thresholds:
        ...
        params['spatial_thresh'] = st
        params['temporal_thresh'] = tt
...
def process_results(params, results, baseline, scale, background, filename):
    ...
    if not params['overlapping']:    # Blockwise Parallel, Single Tiling
        ...
        print_and_flush("Reformating and scaling spatial matrices...")
    ...
    print_and_flush("Writing compressed results file ({})...".format(filename))
    np.savez(filename, 
             U=U, V=V, 
             ...
             allow_pickle=True)
    print_and_flush("Compressed results file successfully written.")
...
if __name__ == "__main__":
    ...
    process_results(params, results, 
                    baseline, scale, 
                    background,
                    os.path.join(outdir, filename + '_compressed.npz'))
